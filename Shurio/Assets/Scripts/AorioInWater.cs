...
public class AorioInWater : MonoBehaviour
{
	public float xpower;
	public float ypower;
...
//		this.gameObject.SendMessage("InWater", true);
	...
	void Update () {
		...
		float inputX = Input.GetAxis("Horizontal_Joy2");
		float inputY = Input.GetAxis("Vertical_Joy2");
		...
		if (waterEnabled && Input.GetAxis("Fire1_Joy2") > 0) {
	...
	void FixedUpdate() {
		...
		float inputX = Input.GetAxis("Horizontal_Joy2");
		float inputY = Input.GetAxis("Vertical_Joy2");
		float jump = Input.GetAxis("Jump_Joy2");
		float enter = Input.GetAxis("Enter_Joy2");
